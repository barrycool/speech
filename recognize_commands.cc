...
RecognizeCommands::RecognizeCommands(const std::vector<string>& labels,
                                     ...
                                     int32_t suppression_ms, int32_t minimum_count)
    : labels_(labels),
...
void RecognizeCommands::ProcessLatestResults(const vector<float>& latest_results,
  ...
  for (const auto& previous_result : previous_results_) {
    const vector<float>& scores_tensor = previous_result.second;
    auto scores_flat = scores_tensor;
